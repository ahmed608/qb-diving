...
local zones = {}
local currentArea = 0
local numridingsuit = 0
local oxygenlevel = 0
local inSellerZone = false
local iswearsuit = false
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1000)
       if iswearsuit == true and oxygenlevel >= Config.oxygenlevel then
            if IsPedSwimmingUnderWater(PlayerPedId()) then
                oxygenlevel = oxygenlevel - 1
                if oxygenlevel == 60 then
                    QBCore.Functions.Notify('You have less than 1 minute of air remaining', 'error')
                    TriggerServerEvent("InteractSound_SV:PlayOnSource", "breathdivingsuit", 0.25)
                elseif oxygenlevel == 0 then
                    QBCore.Functions.Notify('our diving gear is running out of air', 'error')
                    TriggerServerEvent("InteractSound_SV:PlayOnSource", "breathdivingsuit", 0.25)
                    SetPedMaxTimeUnderwater(ped, 50.00)
                elseif oxygenlevel == -40 then
                    deleteGear()
                    SetEnableScuba(ped, false)
                    SetPedMaxTimeUnderwater(ped, 1.00)
                    iswearsuit = false
